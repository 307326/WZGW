%% alg Vincentego, przykładowe dane 
clear;
...
a = 6378137;
e2=0.00669437999013; %do sprawdzenia
...
%x = deg2rad(sigma-sigma_del)
%sab=b*A*(x);

AzAB = atand((cos(Ub)*sin(L))/(cos(Ua)*sin(Ub)-sin(Ua)*cos(Ub)*cos(L)));
AzBA = atand((cos(Ua)*sin(L))/(-sin(Ua)*cos(Ub)+cos(Ua)*sin(Ub)*cos(L)))+180; 
